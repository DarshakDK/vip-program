...
const CART_DETAILS = `query Cart($areaCode: String, $assignCardId: Int, $includeBreakup: Boolean, $buyNow: Boolean, $includeAllItems: Boolean, $includeCodCharges: Boolean, $cartId: String) {
...
export function Component({ props }) {
  ...
  async function fetchCartDetails(fpi) {
    const payload = {
      buyNow: false,
    ...
    return await fpi?.executeGQL?.(CART_DETAILS, payload);
  }
  ...
  const removeCartItem = async (itemDetails, itemSize, itemIndex) => {
    try {
      ...
      if (res?.data?.updateCart?.success) {
        ...
        const updatedCartResponse = await fetchCartDetails(fpi);
        console.log("Updated cart data:", updatedCartResponse);
  ...
  const validateProductPageAccess = (activeCampaign, isVipUser) => {
    ...
    if (
      ...
      console.log("Not on a product page, skipping validation.");
      return;
    }

    const productData = PRODUCT;

    const productItemCode = productData?.product_details?.item_code?.toLowerCase();
...
export const settings = {
  label: "VIP validations product",
  name: "vip-product-validations",
  props: [],
  blocks: [],
};
